...
Func InitBlueStacks5($bCheckOnly = False)
	...
	; BlueStacks accepts the su wrapper only when rooting is enabled for the product
	; and for this instance. Recent versions reset bst.feature.rooting to "0" on every
	; start, and each shell command then returns an empty result and closes the session.
	Local $bRootAccess = True

	For $i = 0 To $iLineCount - 1
		If StringInStr($__Bluestacks5Conf[$i], "bst.feature.rooting=") Then
			If StringInStr($__Bluestacks5Conf[$i], '"1"') = 0 Then $bRootAccess = False
		EndIf
		If StringInStr($__Bluestacks5Conf[$i], "bst.instance." & $g_sAndroidInstance & ".enable_root_access=") Then
			If StringInStr($__Bluestacks5Conf[$i], '"1"') = 0 Then $bRootAccess = False
		EndIf
	...
	If $bInstalled And Not $bCheckOnly Then
		...
		If $bsNow > GetVersionNormalized("5.0") Then
			...
			Else
				; Without rooting the su call returns nothing and closes the ADB session
				$g_sAndroidAdbShellOptions = ""
				; InitBlueStacks5 runs on every Android check, so only report this once
				If Not $g_bBlueStacks5NoRootLogged Then
					SetLog($g_sAndroidEmulator & " root access is disabled, using ADB shell without su", $COLOR_INFO)
					$g_bBlueStacks5NoRootLogged = True
				EndIf
			EndIf
