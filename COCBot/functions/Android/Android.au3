...
Func _WinGetAndroidHandle($bFindByTitle = False)
	...
	; Fallback: locate the window by the class of its rendering control.
	; Required since BlueStacks 5.22, where Win32_Process reports an empty ExecutablePath
	; for HD-Player.exe, so the ProcessExists2() lookup above can never match.
	$hWin = FindAndroidWindowByControlClass()
	If $hWin <> 0 Then
		$t = WinGetTitle($hWin)
		SetDebugLog("Found " & $g_sAndroidEmulator & " Window '" & $t & "' (" & $hWin & ") by control class '" & $g_sAppClassInstance & "'")
		UpdateHWnD($hWin)
		$g_sAndroidTitle = UpdateAndroidWindowTitle($g_hAndroidWindow, $t)
		If $ReInitAndroid = True And $g_bInitAndroid = False Then ; Only initialize Android when not currently running
			$g_bInitAndroid = True ; change window, re-initialize Android config
...
; #FUNCTION# ====================================================================================================================
; Name ..........: FindAndroidWindowByControlClass
; Description ...: Finds the Android Emulator window through the class of its rendering control, without using WMI
; Syntax ........: FindAndroidWindowByControlClass()
; Parameters ....: None
; Return values .: Window handle of the emulator, or 0 when no matching window exists
; Author ........:
; Remarks .......: This file is part of MyBot, previously known as ClashGameBot. Copyright 2015-2025
;                  MyBot is distributed under the terms of the GNU GPL
;                  BlueStacks 5.22 and newer return an empty ExecutablePath in Win32_Process for HD-Player.exe,
;                  which breaks every WMI based process lookup. Scanning top level windows for the rendering
;                  control and confirming the owner with QueryFullProcessImageName still works.
; Link ..........: https://github.com/MyBotRun/MyBot/wiki
; Example .......: No
; ===============================================================================================================================
Func FindAndroidWindowByControlClass()
	If $g_sAppClassInstance = "" Then Return 0

	; Emulator executable name, used to reject windows owned by another application
	Local $sExpectedExe = ""
	If $g_sAndroidProgramPath <> "" Then
		Local $iLastBS = StringInStr($g_sAndroidProgramPath, "\", 0, -1)
		$sExpectedExe = ($iLastBS > 0 ? StringMid($g_sAndroidProgramPath, $iLastBS + 1) : $g_sAndroidProgramPath)
	EndIf
	...
	For $i = 1 To $aWinList[0][0]
		...
		If $sExpectedExe <> "" Then
			Local $iPid = WinGetProcess($hWin)
			If $iPid > 0 Then
				Local $sImage = _WinAPI_GetProcessFileName($iPid)
				If $sImage <> "" And StringInStr($sImage, $sExpectedExe) = 0 Then ContinueLoop
			EndIf
		EndIf

		SetDebugLog("Window candidate by control class: '" & $aWinList[$i][0] & "' (" & $hWin & ") " & $aPos[2] & " x " & $aPos[3])
		If $hFallback = 0 Then $hFallback = $hWin
		; Prefer the window carrying the instance name, required when several instances run
		If $g_sAndroidInstance <> "" And StringInStr($aWinList[$i][0], $g_sAndroidInstance) > 0 Then Return $hWin
	Next

	Return $hFallback
EndFunc   ;==>FindAndroidWindowByControlClass
