...
; CoC 18.600.5 moved the whole end of battle panel about 13 pixels down. Measured on a
; defeat screen: the pale green button top (0xE1F98A) sits at real y 572 instead of 559,
; and the solid body of the Return Home button (0x6CBB1F, exact) spans real y 607 to 611.
Global $aEndFightSceneBtn[4] = [429, 542 + $g_iMidOffsetY, 0xE1F989, 20] ; Victory or defeat scene button = green top of button
; The gold ribbon this used to read is broken up by the Victory / Defeat wording, which
; moves with the length of the word, so only 4 of 14 sampled points matched. The Return
; Home button body is present on both outcomes and measured 13 out of 13 points exact,
; which makes it a far steadier "end scene is rendered" signal.
Global $aEndFightSceneAvl[4] = [400, 579 + $g_iMidOffsetY, 0x6CBB1F, 15] ; Victory or defeat scene = solid green of Return Home button
Global $aEndFightSceneReportGold = $aEndFightSceneAvl ; Missing... TripleM ???
Global $aReturnHomeButton[4] = [430, 579 + $g_iMidOffsetY, 0x6CBB1F, 15] ; Return Home Button, End Battle Screen
...
; The attack window of CoC 18.600.5 is a slide in panel with no close button, so it is
; confirmed by its wooden background below the battle cards. Measured 0x604236 there,
; against 0xD79600 on the village when the panel is closed.
; Coordinates here are written against a 720 pixel tall reference, $g_iBottomOffsetY (60)
; adapts them to the real 780 pixel window, so 580 lands on the measured y = 640.
; Tolerance kept at 10: the attack panel measures exactly 0x604236 there, while the
; nearest other window (Hero Hall, 0x4E3821) sits 21 away, so both sides keep a margin.
Global $aIsAttackPanelOpen[4] = [300, 580 + $g_iBottomOffsetY, 0x604236, 10] ; Attack window panel background, uniform over x 280-320, real y 630-650
; Orange body of the Find a Match button, measured at real (70, 496) on five separate
; captures of the panel, all exactly 0xF9AD2C, while other screens sit 135 or further
; away. It only shows when the panel is open and the button is enabled, which makes it
; a safe guard before clicking the button by position.
Global $aFindMatchButton[4] = [70, 466 + $g_iMidOffsetY, 0xF9AD2C, 20] ; Find a Match button, real button spans x 56-270, y 466-526
; CoC 18.600.5 inserts an army review window between Find a Match and the search, with a
; green Attack! button to confirm. Body measured at real (705, 523), exactly 0x9DE64C,
; while every other screen captured sits 76 or further away.
Global $aArmyWindowAttackButton[4] = [705, 493 + $g_iMidOffsetY, 0x9DE64C, 15] ; green Attack! button, real button spans x 655-800, y 500-550
...
;returnhome
; Same 13 pixel shift of the end of battle panel: both colours now sit at real y 600
; (measured 0x76C324 at x 363 and 0x76C324 at x 497, inside a stable band from y 593 to 615).
Global Const $aRtnHomeCheck1[4] = [363, 570 + $g_iMidOffsetY, 0x78C11C, 20]
Global Const $aRtnHomeCheck2[4] = [497, 570 + $g_iMidOffsetY, 0x79C326, 20]
