...
Func IsMultiplayerTabOpen()
	Local $aMultiplayerTab = findImage("IsMultiplayerTab", $g_sImgIsMultiplayerTab, GetDiamondFromRect("65,500,260,650"), 1, True, Default)
...
Func IsReturnHomeBattlePage($useReturnValue = False, $makeDebugImageScreenshot = True)
	...
	; CoC 18.600.5 moved the end of battle panel down, so the white lettering of the button
	; now sits at real y 585 to 589 instead of 575. Reading 545 + offset landed on the pale
	; green gradient above the text, which made this double check fail every single time
	; and stopped the Return Home click from ever being issued.
	If IsPageLoop($aReturnHomeButton, 1) Then
		If _ColorCheck(_GetPixelColor($aReturnHomeButton[0], 557 + $g_iMidOffsetY, True), Hex(0xFFFFFF, 6), 10) Then ; White Color From Button (Double Check)
	...
	If IsPageLoop($aRewardButton, 1) Then
		If _ColorCheck(_GetPixelColor($aRewardButton[0] + 8, 557 + $g_iMidOffsetY, True), Hex(0xFFFFFF, 6), 10) Then ; White Color From Button (Double Check)
...
Func IsPostDefenseSummaryPage($bCapture = True)
	...
	$result = _ColorCheck($GoldSpot, Hex(0xE4DB4F, 6), 20) And _ColorCheck($ElixirSpot, Hex(0xDA38D1, 6), 20)
