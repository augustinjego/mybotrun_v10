...
Func UpgradeHeroes()
	...
	; Recent CoC versions moved the close button of this window to the top right corner
	; (measured around 815, 190), so search the whole upper right quadrant like
	; CloseWindow() does instead of the narrow box the old layout used (780,145 - 815,175).
	; The window can also take longer than the fixed wait above to appear, so retry.
	Local $bHeroHallOpen = False
	For $iHeroHallTry = 1 To 5
		If QuickMIS("BC1", $g_sImgGeneralCloseButton, Round($g_iGAME_WIDTH / 2), 0, $g_iGAME_WIDTH, 460 + $g_iMidOffsetY) Then
			$bHeroHallOpen = True
			ExitLoop
		EndIf
		If _Sleep(700) Then Return
	Next
	If Not $bHeroHallOpen Then
