...
Func checkChatTabPixel()
	...
	Else
		If Not $g_bRunState Then Return False
		; CoC replaced the left hand chat tab with the Social button, so this reference
		; image can no longer match. The main screen is already confirmed by the pixel
		; check and the obstacle check, so a miss here must not block the bot.
		If Not $g_bChatTabPixelMissingLogged Then
			SetLog("ChatTabPixel not found, skipping this check (CoC UI changed)", $COLOR_INFO)
			$g_bChatTabPixelMissingLogged = True
		EndIf
		Return True
	EndIf
